...
const { runPatchMigrations } = require('./patchMigrations')
...
// ponytail: archived 列补丁已迁至 patchMigrations.js 的 p01（2026-08-21 阶段 2 收敛）
...
async function syncModels() {
  ...
  // ensureMemoryFts 不走 patchMigrations：FTS5 表与 SessionSummary/ChatHistory 的 afterSync
  // hook 联动重建（两处保持一致），且其幂等检测依赖列结构而非记录，纳入迁移框架风险大于收益
  ...
  // 历史补丁迁移（p01~p09：archived 列/c30 绑定/材料批次/试配用量/幂等键/索引等）
  // 原 syncModels 内联的 19 处 ensureColumn + 3 处 CREATE INDEX + daily_plans 重建已收敛到
  // patchMigrations.js；新 schema 变更一律在那里新增 pXX，不再往本函数堆代码
  await runPatchMigrations(sequelize)
