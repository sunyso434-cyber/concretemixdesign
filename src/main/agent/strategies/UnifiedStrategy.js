...
// Task 2.3: 提升到模块级常量，供 _executeSingleTool helper 复用（原为 execute() 局部 const）
...
// 注意：用户消息已由前端 agentActions.js 保存，此处不再重复保存
   ...
   */
  _buildCachedToolMsg(toolCallId, rawContent, cacheResult) {
   ...
   */
  _emitToolCompletion(r, { mode, roundIndex }) {
   ...
   */
  async _executeSingleTool(tc, ctx) {
   ...
   */
  async _executeToolCalls(response, { trimmedMessages, failureCounters, softWarnSent, mode, roundIndex }) {
