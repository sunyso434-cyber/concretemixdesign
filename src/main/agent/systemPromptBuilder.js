...
- workspace_writeFile({ type, filename, payload }) → 写 md 到 reports/（仅支持 md 格式。xlsx 报表请用 generate_xlsx_report；docx 及编辑已有文件请用 officecli 系列：create_office_file + edit_office_file）
...
**xlsx 报表分工（重要）**：结构化数据从零造表（报价单/材料汇总/试验数据表）→ **必须用 \`generate_xlsx_report\`**，一次调用出专业样式成品（标题合并、表头冻结、斑马纹、数字格式、总计行求和），不要用 officecli 逐格搭建。
写 docx / 编辑已有 Office 文件 → 用 officecli 系列：\`create_office_file\` 创建空白文档 → \`edit_office_file\` 批量添加内容（支持 add/set/add_table/replace，一次最多50个操作，支持 font.ea/font.latin/size/bold/firstLineIndent/lineSpacing 等完整格式属性） → \`read_office_file\` 校验。模板场景用 \`merge_office_template\`。
