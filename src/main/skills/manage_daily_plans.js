...
module.exports = {
  ...
  category: 'manage',
  description: `管理每日生产调度计划(CRUD)。action: create|update|delete|get。同一天同工程同部位同标号同站只能一条。匹配键字段(planDate/projectName/pourLocation/strengthGrade/branchId)不可修改。配合比由分公司配置自动绑定，计划无需传mixDesignId。有车次时不可删除(用forceDelete=true强制删，会置车次planId=NULL)。

正确调用示例（所有字段作为平级参数直接传，不要包data对象）:
{"action":"create","planDate":"2026-08-06","projectName":"某某项目","pourLocation":"某某部位","strengthGrade":"C30","branchId":1,"volume":100,"startTime":"08:00","duration":4}`,
  version: '1.1.0',
  parameters: {
    type: 'object',
    properties: {
      ...
      // 字段名取 AI 直觉名（startTime/duration），execute 内部映射到模型字段
  ...
  async execute(args, context) {
    ...
    const FIELDS = ['planDate', 'projectName', 'constructionUnit', 'pourLocation', 'receiveMethod', 'strengthGrade', 'volume', 'branchId', 'plannedSendTime', 'expectedDuration', 'remarks']
    const NUM_FIELDS = ['volume', 'branchId', 'expectedDuration']
    // 字段别名（AI 从表格直译的列名 → 标准字段）：startTime/duration/notes
    const ALIAS = { startTime: 'plannedSendTime', duration: 'expectedDuration', notes: 'remarks' }
