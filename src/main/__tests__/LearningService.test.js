...
describe('LearningService 集成 PreferencePatternDetector', () => {
  ...
  test('应监听 tool:executed 事件并把 5 次相同材料写入 suggestionStore', async () => {
    for (let i = 0; i < 5; i++) {
    ...
    expect(
      list.some(s => s.type === 'material_vendor' && s.payload?.proposedYaml?.value === '拉法基')
    ).toBe(true)
  })

  test('不应对非 calculate_mix_design 工具进行观察', async () => {
    for (let i = 0; i < 5; i++) {
  ...
  test('不应在 result.success === false 时观察', async () => {
    for (let i = 0; i < 5; i++) {
