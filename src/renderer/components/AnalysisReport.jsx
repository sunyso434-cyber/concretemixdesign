/**
 * AI 分析报告组件（从 AIAnalysisPage_Results 迁移）
 *
 * 渲染 AI 返回的配合比分析报告，包含：
 * - 综合分析（材料性能影响、配合比参数影响、最优配合比、调整建议、试验建议）
 * - 趋势分析 Tab（参数影响回归）
 * - 材料对比 Tab（多组材料对比）
 */

import React, { useState } from 'react'
import { Card, Divider, Space, Alert, Tabs, Empty, Table, Row, Col, Select, List } from 'antd'
                ...
                {fts.alternativeDirections && fts.alternativeDirections.length > 0 && (
                  <>
                    ...
                    {fts.alternativeDirections.map((dir, i) => (
                      <Alert key={i} type="info" message={dir} style={{ marginBottom: 8 }} />
                    ))}
                  </>
                )}
              </>
